...
- 不能在pages里面使用别的文件，比如`hooks.ts、const.ts`，开发不会有错，打包会有错
- 如果本地有图片要放在public目录下，引入的时候直接`/xxx.jpg`
- 如`getServerSideProps`在服务端上运行代码console只能在终端看到

## 搭建

官网搭建后，需要自己集成prettier，husky等（参考react搭建就可以），eslint规则不是很够，新增几个实用的
...
> 这几个函数都只能在页面组件使用，老版本9.3之前是用`getInitialProps`，新版本用`getServerSideProps`
