...
3.  回滚到某次提交：`git revert commitid`
    ...
    - 强制更新`git push -f`
4.  强制更新：`git push -f`
5.  合并其他分支所有提交信息（为一次提交）到当前分支：`git cherry-pick committed`
6.   缓存文件，在一个分支没有开发完，要到另一个分支去处理问题，用 stash 缓存：`git stash`
7.  
    上线一个版本可能需要打 tag：`git tag v0.1`，推送tag用：`git push origin tag v0.1`
8.  修改上次提交的 commit 信息： `git commit --amend`，输入：`git commit --amend`，然后按vim模式保
